# Maven Test Fixtures Polyglot Plugin

The **Maven Test Fixtures Polyglot Plugin** orchestrates the creation, packaging, and publishing of Java test fixtures in a Maven build. It acts as a shadow build system within the standard Maven lifecycle, managing an isolated compilation phase, constructing a synthetic POM in memory, and bypassing the standard deployer to push multiple distinct artifact graphs to the repository.
...
### **3. Achieving Seamless Gradle Interoperability (Future goal)**

Publishing `artifact-test-fixtures.pom` makes it usable in Gradle via `testImplementation("group:artifact-test-fixtures:1.0")`. However, true seamless interoperability means a Gradle user should be able to type `testFixtures("group:artifact:1.0")` and have it resolve automatically.

To achieve this, the `deploy-polyglot` Mojo can be extended to generate one final file: **Gradle Module Metadata (`.module`)**.
During the packaging phase, the plugin would generate a JSON file formatted to Gradle's specification, defining variants like `apiElements` and `testFixturesApiElements`. When Gradle resolves the main artifact, it reads the `.module` file, sees the capability, and routes the dependency graph accurately. Maven consumers ignore the `.module` file and natively resolve the synthetic POMs deployed.
